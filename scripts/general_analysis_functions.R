...
estimate_weights <- function(data, baseline_covariates,
  ...
  prob_living_donor_transplant <- 
    predict(glm(formula=as.formula(paste("transplant_and_living_donor", transplant_formula_RHS, sep=" ~ ")), 
                data=data %>% filter(censoring_event==0 & post_transplant==0), 
                family=binomial()), 
            newdata=data, type="response")
  
  prob_deceased_donor_transplant <- 
    predict(glm(formula=as.formula(paste("transplant_and_deceased_donor", transplant_formula_RHS, sep=" ~ ")), 
                data=data %>% filter(censoring_event==0 & post_transplant==0 & transplant_and_living_donor==0), 
                family=binomial()), 
            newdata=data, type="response")
  ...
  data_with_weights <- data.frame(patient_id = data$patient_id,
                                  ...
                                  transplant_and_living_donor = data$transplant_and_living_donor,
                                  transplant_and_deceased_donor = data$transplant_and_deceased_donor,
                                  prob_censored = prob_censored,
                                  prob_living_donor_transplant = prob_living_donor_transplant,
                                  prob_deceased_donor_transplant = prob_deceased_donor_transplant) %>%
    mutate(
           Wt_censoring = case_when(censoring_event == 1 ~ 0,
                                    TRUE ~ 1/(1-prob_censored)),
           Wt_d = 1,
           Wt_dd = 1,
           Wt_no_int = Wt_censoring
           ) %>%
    group_by(patient_id) %>%
  ...
  data_with_weights <- adjust_wts(
    ...
    mutate(Wt_g = Wt_censoring*Wt_d*Wt_dd) %>%
    group_by(patient_id) %>% 
...
adjust_wts <- function(data_with_weights, living_donor_usage_factor, deceased_donor_usage_factor=1) {
  ...
  for (j in 1:length(alpha)) {
    ...
    data_with_weights <- data_with_weights %>% mutate(Wt_g = Wt_censoring*Wt_d*Wt_dd,
    ...
    {sum(.$transplant_and_living_donor*(1-.$censoring_event)*.$Wt_no_int_cumprod)}
    
    E_B_k_g_natural_B <- data_with_weights %>% filter(timegroup==j) %>% 
    {sum(.$transplant_and_living_donor*(1-.$censoring_event)*.$Wt_g_natural_B_cumprod)}
    
    E_Y_k_g_natural_B <- data_with_weights %>% filter(timegroup==j) %>% 
    {sum((1-.$post_transplant)*(1-.$censoring_event)*.$Wt_g_natural_B_cumprod)}
    
    alef_B[j] <- (living_donor_usage_factor*E_B_k <= E_Y_k_g_natural_B)
    
    bet_B[j] <- (E_B_k_g_natural_B > living_donor_usage_factor*E_B_k)
    
    if (alef_B[j] == 0) {
      alpha[j] <- 0
    } else {
      if (bet_B[j] == 1) {
        alpha[j] <- living_donor_usage_factor*E_B_k/E_B_k_g_natural_B
      } else {
        alpha[j] <- (1-living_donor_usage_factor*(E_B_k/E_Y_k_g_natural_B))/(1-(E_B_k_g_natural_B/E_Y_k_g_natural_B))
      }
    }
    ...
    data_with_weights <- data_with_weights %>% mutate(
      Wt_d = case_when(post_transplant == 1 ~ 1,
                       transplant_and_living_donor == 1 ~ (alpha[timegroup]^bet_B[timegroup])*
                         ((1-alpha[timegroup]*(1-prob_living_donor_transplant))/prob_living_donor_transplant)^(1-bet_B[timegroup]),
                       TRUE ~ ((1-alpha[timegroup]*prob_living_donor_transplant) / (1-prob_living_donor_transplant))^(bet_B[timegroup])* 
                         (alpha[timegroup]^(1-bet_B[timegroup]))),
      Wt_g = Wt_censoring*Wt_d*Wt_dd,
    ...
    {sum(.$transplant_and_deceased_donor*(1-.$censoring_event)*(1-.$transplant_and_living_donor)*.$Wt_no_int_cumprod)}
    
    E_H_k_g_natural_H <- data_with_weights %>% filter(timegroup==j) %>% 
    {sum(.$transplant_and_deceased_donor*(1-.$censoring_event)*(1-.$transplant_and_living_donor)*.$Wt_g_natural_H_cumprod)}
    
    E_Z_k_g_natural_H <- data_with_weights %>% filter(timegroup==j) %>% 
    {sum((1-.$post_transplant)*(1-.$censoring_event)*(1-.$transplant_and_living_donor)*.$Wt_g_natural_H_cumprod)}
    
    alef_H[j] <- (deceased_donor_usage_factor*E_H_k <= E_Z_k_g_natural_H)
    
    bet_H[j] <- (E_H_k_g_natural_H > deceased_donor_usage_factor*E_H_k)
    
    if (alef_H[j] == 0) {
      beta[j] <- 0
    } else {
      if (bet_H[j] == 1) {
        beta[j] <- deceased_donor_usage_factor*E_H_k/E_H_k_g_natural_H
      } else {
        beta[j] <- (1-deceased_donor_usage_factor*(E_H_k/E_Z_k_g_natural_H))/(1-(E_H_k_g_natural_H/E_Z_k_g_natural_H))
      }
    }
    ...
    data_with_weights <- data_with_weights %>% mutate(
      Wt_dd = case_when(post_transplant == 1 | transplant_and_living_donor == 1 ~ 1,
                        transplant_and_deceased_donor == 1 ~ (beta[timegroup]^bet_H[timegroup])*
                          ((1-beta[timegroup]*(1-prob_deceased_donor_transplant))/prob_deceased_donor_transplant)^(1-bet_H[timegroup]),
                        TRUE ~ ((1-beta[timegroup]*prob_deceased_donor_transplant)/(1-prob_deceased_donor_transplant))^(bet_H[timegroup])* 
