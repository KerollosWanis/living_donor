    ...
    primary_diagnosis = as.factor(
      case_when(
        ...
        Primary.Diagnosis %in% c(
          ...
          "4272: LI:BILIARY HYPOPLASIA: ALAGILLE\x92S SYNDROME (PAUCITY OF INTRAHEPATIC BILE DUCT)",
...
candidates_analysis <-
  candidates_expanded %>% group_by(patient_id) %>%  mutate(
    ...
    transplant_and_living_donor = case_when(transplant_event == 1 & deceased_donor == 'living' ~ 1,
                                         TRUE ~ 0),
    transplant_and_deceased_donor = case_when(transplant_event == 1 & deceased_donor == 'deceased' ~ 1,
                                         TRUE ~ 0))
...
candidates_analysis <-
  candidates_analysis %>% dplyr::select(
    c(
      ...
      transplanted,
      transplant_event,
      transplant_and_living_donor,
      transplant_and_deceased_donor,
      transplant_date,
      post_transplant,
